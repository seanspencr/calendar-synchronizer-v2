...
def plot_confusion_matrix(y_true, y_pred, labels, filepath):
    ...
    plt.title("Naive Bayes — Token-Level Confusion Matrix")
...
def plot_f1_per_class(class_f1, filepath):
    ...
    plt.xlim(0, 1)
    plt.xlabel("F1 Score")
    plt.title("Naive Bayes — Per-Class F1 (Entity Tags)")
...
def main():
    ...
    plot_confusion_matrix(y_test_flat, y_pred, ALL_BIO_LABELS,
                          os.path.join(PLOTS_DIR, "nb_confusion_matrix.png"))
    ...
    plot_f1_per_class(entity_f1, os.path.join(PLOTS_DIR, "nb_f1_per_class.png"))
    ...
    print("-" * 55)
    print(f"{'Naive Bayes':15s} | {accuracy*100:6.2f}%  | {macro_p:6.3f}  | {macro_r:6.3f}  | {macro_f1:6.3f}")

    model = {"clf": clf, "vec": vec, "le": le}
    os.makedirs("output", exist_ok=True)
    with open("output/naive_bayes.pkl", "wb") as f:
        pickle.dump(model, f)
    print("Model saved to output/naive_bayes.pkl")
